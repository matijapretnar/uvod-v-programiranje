...
kljuci = sorted(pojavitve)

import math

def druga_komponenta(par):
    return par[1]

def padajoce_po_drugi_narascajoce_po_prvi_padajoce_po_tretji(nabor):
    return (-nabor[1], nabor[0], -nabor[2])

print(sorted(
    [(kljuc, pojavitve[kljuc]) for kljuc in kljuci],
    key=lambda p: p[1]
))

print(sorted(pojavitve.items(), key=lambda p: p[1]))
