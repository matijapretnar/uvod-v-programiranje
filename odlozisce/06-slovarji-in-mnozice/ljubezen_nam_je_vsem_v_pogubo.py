...
def narcisoidi(omrezje):
    narcisi = set()
    for oseba, ljubljeni in omrezje.items():
...
def ljubljeni(omrezje):
    vsi_ljubljeni = set()
    for ljubljeni_osebe in omrezje.values():
        vsi_ljubljeni |= ljubljeni_osebe
...
def pari(omrezje):
    vsi_pari = set()
    for oseba, ljubljeni in omrezje.items():
        for ljubezen in ljubljeni:
            if oseba in omrezje[ljubezen] and oseba <= ljubezen:
                vsi_pari.add((oseba, ljubezen))
    return vsi_pari
...
def zaljubljeni_v_osebo(oseba, zaljubljeni):
    zaljubljeni_v = set()
    for k, v in zaljubljeni.items():
        if oseba in v:
            zaljubljeni_v.add(k)
    return zaljubljeni_v


def ustrezljivi(oseba, zaljubljeni):
    vsi_ustrezljivi = set()
    novi = zaljubljeni_v_osebo(oseba, zaljubljeni)
    while novi:
        vsi_ustrezljivi |= novi
        novi_novi = set()
        for nov in novi:
            novi_novi |= zaljubljeni_v_osebo(nov, zaljubljeni)
        novi = novi_novi - vsi_ustrezljivi
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoxMzg0LCJ1c2VyIjo4MH0:1lQqjA:4AZnymNK06QFqqUkO_sAlGuA5xA'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoxMzg1LCJ1c2VyIjo4MH0:1lQqjA:wAInEHLTqmB3tyPkbr3Qm4rUX_g'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoxMzg2LCJ1c2VyIjo4MH0:1lQqjA:QtsWV-eEnLek5UAFreJINgSffEQ'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoxMzg3LCJ1c2VyIjo4MH0:1lQqjA:MaPVaBEbp38pIdEvpZWmzoZlXD0'
        try:
    ...
    try:
        ...
        token = 'Token 67d1e20689c3458677e4832ae8bf4d51092d64c0'
